  ...
  useEffect(() => {
    ...
    const resolvedColors =
      colors && colors.length > 0
        ? colors
        : [color0 ?? defaultColors[0], color1 ?? defaultColors[1], color2 ?? defaultColors[2]];
    const paletteTex = makePaletteTexture(resolvedColors);
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        this.uniforms = (this.props.material as { uniforms?: Uniforms } | undefined)?.uniforms;
      }
      ...
      init(...args: unknown[]) {
        ...
        if (this.uniforms) {
          this.material = new THREE.RawShaderMaterial(
            this.props.material as ConstructorParameters<typeof THREE.RawShaderMaterial>[0],
          );
      ...
      update() {
        ...
        Common.renderer.setRenderTarget(
          (this.props.output as THREE.WebGLRenderTarget | null | undefined) ?? null,
        );
    ...
    class Advection extends ShaderPass {
      ...
      update(opts?: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
        ...
        if (typeof opts?.dt === "number") this.uniforms.dt.value = opts.dt;
        if (typeof opts?.isBounce === "boolean") this.line.visible = opts.isBounce;
        if (typeof opts?.BFECC === "boolean") this.uniforms.isBFECC.value = opts.BFECC;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(opts?: { mouse_force?: number; cellScale?: THREE.Vector2; cursor_size?: number }) {
        const forceX = (Mouse.diff.x / 2) * (opts?.mouse_force ?? 0);
        const forceY = (Mouse.diff.y / 2) * (opts?.mouse_force ?? 0);
        const cellScale = opts?.cellScale || new THREE.Vector2(1, 1);
        const cursorSize = opts?.cursor_size ?? 0;
    ...
    class Viscous extends ShaderPass {
      ...
      update(opts?: { viscous?: number; iterations?: number; dt?: number }) {
        ...
        if (typeof opts?.viscous === "number") this.uniforms.v.value = opts.viscous;
    ...
    class Divergence extends ShaderPass {
      ...
      update(opts?: { vel?: THREE.WebGLRenderTarget }) {
        if (this.uniforms && opts?.vel) {
    ...
    class Pressure extends ShaderPass {
      ...
      update(opts?: { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget }) {
        if (this.uniforms && opts?.vel && opts?.pressure) {
          this.uniforms.velocity.value = opts.vel.texture;
    ...
    class Simulation {
      ...
      createShaderPass() {
        this.advection = new Advection({
          ...
          dt: this.options.dt,
          src: this.fbos.vel_0 as THREE.WebGLRenderTarget,
          dst: this.fbos.vel_1 as THREE.WebGLRenderTarget,
        });
        ...
        this.viscous = new Viscous({
          ...
          src: this.fbos.vel_1 as THREE.WebGLRenderTarget,
          dst: this.fbos.vel_viscous1 as THREE.WebGLRenderTarget,
          dst_: this.fbos.vel_viscous0 as THREE.WebGLRenderTarget,
          dt: this.options.dt,
        });
        this.divergence = new Divergence({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: this.fbos.vel_viscous0 as THREE.WebGLRenderTarget,
          dst: this.fbos.div as THREE.WebGLRenderTarget,
          dt: this.options.dt,
        });
        this.poisson = new Poisson({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: this.fbos.div as THREE.WebGLRenderTarget,
          dst: this.fbos.pressure_1 as THREE.WebGLRenderTarget,
          dst_: this.fbos.pressure_0 as THREE.WebGLRenderTarget,
        });
        this.pressure = new Pressure({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src_p: this.fbos.pressure_0 as THREE.WebGLRenderTarget,
          src_v: this.fbos.vel_viscous0 as THREE.WebGLRenderTarget,
          dst: this.fbos.vel_0 as THREE.WebGLRenderTarget,
          dt: this.options.dt,
        });
      }
      ...
      update() {
        ...
        let vel = this.fbos.vel_1 as THREE.WebGLRenderTarget | undefined;
